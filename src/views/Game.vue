	...
	<div class="h-screen w-screen flex items-center justify-center bg-ttt">
		<div v-if="ticTacToeStore.gameState" class="flex flex-col justify-center items-center z-10">
			...
			<div class="ttt-subtext flex flex-row justify-center items-center">
				...
				<button class="flex flex-row cursor-pointer items-center px-2 underline t-orange" @click="copy()">
					{{ copied ? "Copied" : ticTacToeStore.gameID }}
					<img :src="copySvg" class="ttt-icon-piece mx-2" alt="copy" />
				</button>
				/
				<button class="flex flex-row cursor-pointer items-center px-2 underline t-orange" @click="linkCopy()">
			...
			<p v-if="isSpectator" class="ttt-subtext flex flex-row items-center ttt-turn">
				...
				<img :src="currentPiece" class="ttt-icon-piece ml-[0.69rem]" :alt="pieceValue" />
			</p>
			<p
		...
		<div class="flex h-full w-full absolute px-14 py-5">
			<div class="relative flex h-full w-full">
				<button @click="leaveMatch">
					<img :src="exitSvg" class="ttt-icon absolute top-0 left-0 cursor-pointer" alt="exit" />
				</button>
				<volumeButton />
			</div>
		</div>
